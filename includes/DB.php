...
$GLOBALS['platform_settings'] = Cache::get('platform_settings');

if ($GLOBALS['platform_settings'] === null) {
    try {
        $stmt = $pdo->query("SELECT setting_key, setting_value FROM cp_settings");
        $GLOBALS['platform_settings'] = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
        Cache::set('platform_settings', $GLOBALS['platform_settings'], 300); // 5 min TTL
    } catch (\Exception $e) {
        $GLOBALS['platform_settings'] = [];
    }
}

$platform_settings = $GLOBALS['platform_settings'];
