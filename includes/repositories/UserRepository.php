...
class UserRepository {
    ...
    public function getAll(): array {
        $stmt = $this->pdo->prepare("
            SELECT u.*, creator.name as creator_name, c.name as company_name
            FROM cp_users u 
    ...
    public function save(array $data): bool {
        if (isset($data['id']) && $data['id']) {
            $sql = "UPDATE cp_users SET name = ?, email = ?, role = ?, company_id = ?, city = ?, state = ?, zip_code = ?, street = ?, neighborhood = ?, address_number = ?, phone = ?, avatar = ?";
            $params = [
                $data['name'], 
                $data['email'], 
                $data['role'], 
            ...
            $sql = "INSERT INTO cp_users (name, username, email, password, role, company_id, created_by, city, state, zip_code, street, neighborhood, address_number, phone, avatar) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $params = [
                $data['name'], 
                $data['username'],
                $data['email'], 
