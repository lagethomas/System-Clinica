...
<div class="card">
    <div class="table-responsive">
        <table class="premium-table">
            ...
            <tbody>
                <?php if (empty($plans)): ?>
                    <tr><td colspan="7" class="text-center" style="padding: 40px; color: var(--text-muted);">Nenhum pacote cadastrado.</td></tr>
                <?php else: ?>
    ...
    <?php echo \App\Core\Pagination::render($pagination['page'], $pagination['totalPages'], SITE_URL . '/admin/plans'); ?>
</div>
...
async function savePlan(e, form) {
    ...
    if (submitBtn) {
        ...
        submitBtn.innerHTML = '<i data-lucide="loader" class="icon-lucide"></i> Salvando...';
...
async function deletePlan(id) {
    if (await UI.confirm('Deseja realmente excluir este pacote? Empresas associadas ficarão sem pacote definido.')) {
        const result = await UI.request('<?php echo SITE_URL; ?>/api/admin/plans/delete', { 
            id, 
