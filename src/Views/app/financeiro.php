    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 20px;">
        <form action="" method="GET" style="display: flex; gap: 20px; align-items: center; flex-wrap: wrap;">
            ...
            <button type="submit" class="btn-primary" style="padding: 8px 18px; font-size: 13px; border-radius: 8px;">
                <i data-lucide="filter" class="icon-lucide"></i> Aplicar
            </button>
        </form>

        <div class="d-flex gap-2 align-items-center">
            <button class="btn-finance-action btn-finance-saida" onclick="openFinanceModal('saida')">
                <i data-lucide="minus-circle" class="icon-lucide"></i> <span class="d-none d-md-inline">Nova</span> Saída
            </button>
            <button class="btn-finance-action btn-finance-entrada" onclick="openFinanceModal('entrada')">
                <i data-lucide="plus-circle" class="icon-lucide"></i> <span class="d-none d-md-inline">Nova</span> Entrada
            </button>
            <?php if (Auth::isProprietario()): ?>
            <button class="btn-finance-action" style="background: var(--accent); color: #fff; border: none; border-radius: 10px; padding: 10px 18px; font-weight: 700; display: flex; align-items: center; gap: 8px; font-size: 13px; box-shadow: 0 4px 12px rgba(var(--accent-rgb), 0.2);" onclick="openFinanceModal('cobranca')">
                <i data-lucide="circle-dollar-sign" class="icon-lucide"></i> <span class="d-none d-md-inline">Lançar</span> Cobrança
            </button>
            <?php endif; ?>
        </div>
    </div>
...
<div class="finance-stats-grid">
    <div class="finance-card entrada">
        <div class="finance-icon"><i data-lucide="arrow-up" class="icon-lucide"></i></div>
    ...
    <div class="finance-card saida">
        <div class="finance-icon"><i data-lucide="arrow-down" class="icon-lucide"></i></div>
        <div class="finance-info">
    ...
    <div class="finance-card saldo">
        <div class="finance-icon"><i data-lucide="coins" class="icon-lucide"></i></div>
        <div class="finance-info">
...
<div class="finance-main-grid">
    ...
    <div class="card">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 25px;">
            <h3 style="font-size: 16px; margin:0;"><i data-lucide="trending-up" class="icon-lucide"></i> Tendência de Fluxo (7 dias)</h3>
    ...
    <div class="card" style="padding:0;">
        <div style="padding: 20px; border-bottom: 1px solid var(--border);">
             <h3 style="font-size: 16px; margin:0;"><i data-lucide="history" class="icon-lucide"></i> Últimas Movimentações</h3>
        </div>
        <div class="table-responsive" style="display: block; width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch;">
            <table class="premium-table" style="min-width: 900px !important;">
                <thead>
                    <tr>
                        <th>Data</th>
                        <th>Descrição</th>
                ...
                <tbody>
                    <?php if (empty($movimentacoes)): ?>
                        <tr><td colspan="5" class="text-center" style="padding: 30px; color: var(--text-muted);">Sem registros.</td></tr>
                    <?php else: ?>
                        <?php foreach ($movimentacoes as $m): ?>
                            <tr>
                                ...
                                <td><span class="badge" style="background: rgba(var(--primary-rgb), 0.1); color: var(--primary); border: 1px solid rgba(var(--primary-rgb), 0.2); font-size: 11px;"><?php echo htmlspecialchars($m['categoria'] ?? 'Geral'); ?></span></td>
                                ...
                                <td class="text-right">
                                    <button onclick="deleteFinanceiro(<?php echo $m['id']; ?>)" class="btn-user-action btn-user-delete"><i data-lucide="trash-2" class="icon-lucide"></i></button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
...
function openFinanceModal(tipo = 'entrada') {
    ...
    const tipoFinal = isCobranca ? 'entrada' : tipo;
    const colorClass = isCobranca ? 'accent' : (tipo === 'entrada' ? 'entrada' : 'saida');
    const modalTitle = isCobranca ? 'Lançar Cobrança (Cliente)' : (tipo === 'entrada' ? 'Nova Entrada' : 'Nova Saída');
        ...
        <form class="ajax-form" id="form-finance" action="<?php echo SITE_URL; ?>/api/financeiro/add">
            <div class="modal-body-scroll" style="padding: 10px;">
                ...
                <div class="form-group mb-3">
                    <label class="form-label">Buscar Cliente (Tutor)</label>
                    <select name="tutor_id" class="form-control w-100 searchable-select" required style="padding: 12px; border-radius: 10px;">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" name="descricao" class="form-control" required placeholder="${isCobranca ? 'Ex: Mensalidade, Consulta Particular...' : 'Ex: Venda de Produto, Aluguel...'}">
                </div>

                <div class="form-group mb-3">
                    <label class="form-label">Categoria</label>
                    <select name="categoria" class="form-control w-100" style="padding: 12px; border-radius: 10px; border: 1px solid var(--border); background: var(--bg-surface); color: var(--text-main);">
                        ${isCobranca ? '<option value="Cobrança">Cobrança</option>' : ''}
            ...
            <div class="modal-footer" style="padding-top:20px; border-top: 1px solid var(--border);">
                ...
                <button type="submit" class="btn-finance-action btn-finance-${colorClass}"><i data-lucide="save" class="icon-lucide icon-sm mr-2"></i> Salvar Lançamento</button>
            </div>
        </form>
    `;

    UI.showModal(modalTitle, html);
