    ...
    public function index(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($_POST['save_general'])) {
                ...
                if (isset($_FILES['system_logo']) && $_FILES['system_logo']['error'] === UPLOAD_ERR_OK) {
                    ...
                    $uploadDir = dirname(dirname(dirname(__DIR__))) . '/public/uploads/system/';
                    if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
                    $uploadPath = $uploadDir . $filename;
                    
                    if ($this->convertToWebP($tmpPath, $uploadPath)) {
                        $val = '/uploads/system/' . $filename;
            ...
            if (isset($_POST['save_billing'])) {
                if (!\Nonce::verify($nonce, 'save_billing_settings')) {
                    header("Location: " . SITE_URL . "/settings?tab=billing&msg=error_nonce");
                    exit;
                }
                $keys = ['grace_period', 'days_before_notify'];
                foreach ($keys as $key) {
                ...
                Cache::delete('platform_settings');
                header("Location: " . SITE_URL . "/settings?tab=billing&msg=saved");
                exit;
            }
        }
        ...
        $this->render('admin/settings', [
            ...
            'nonces' => [
                ...
                'billing' => \Nonce::create('save_billing_settings')
            ]
        ]);
    }
